...
export async function reconcileCommentAttachmentDerivative(
  db: D1Database,
  submissionId: string,
  ...
  const pair = await db.prepare(`
    SELECT candidate.source_sha256,
           candidate.source_byte_size,
           candidate.generator_version,
           candidate.derived_asset_id AS preview_asset_id
    FROM attachment_derivative_comment_candidates candidate
    WHERE candidate.submission_id = ?
      AND (? = candidate.preview_item_id OR ? = candidate.original_item_id)
    LIMIT 1
  `).bind(submissionId, changedItemId, changedItemId).first<{
    source_sha256: string;
    source_byte_size: number;
    generator_version: string;
  ...
  return registerReadyAttachmentDerivative(db, {
    sourceSha256: pair.source_sha256,
    sourceByteSize: Number(pair.source_byte_size),
    generatorVersion: pair.generator_version,
    derivedAssetId: pair.preview_asset_id,
    actorEmail,
  }, now);
}
