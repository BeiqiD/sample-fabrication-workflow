...
describe("parameterized browser-preview MIME", () => {
  ...
  it("automatically adopts a ready Comment TIFF pair with parameterized WebP preview MIME", () => {
    const database = referenceTestDatabase();
    ...
    expect(database.prepare(`
      SELECT source_sha256, generator_version, derived_asset_id, status
      FROM attachment_derivatives
      WHERE source_sha256 = ?
    `).get(sourceSha)).toEqual({
      source_sha256: sourceSha,
      generator_version: COMMENT_TIFF_PREVIEW_GENERATOR_VERSION,
      derived_asset_id: "comment-parameterized-preview",
      status: "ready",
    });
    expect(database.prepare(`
      SELECT COUNT(*) AS count
