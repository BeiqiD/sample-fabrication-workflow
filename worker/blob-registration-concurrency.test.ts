...
async function uploadRequest(bytes: Uint8Array, kind: "comment_image" | "attachment", sha256?: string) {
  ...
  headers["x-content-sha256"] = sha256 ?? await sha256Hex(bytesBuffer(bytes));
  return new Request(
...
async function runConcurrentR2Uploads(
  ...
  await acceptCommentUpload(database, env, { kind: "comment_image", bytes: firstBytes });
  const first = worker.fetch(
    await uploadRequest(firstBytes, "comment_image"),
    env,
    executionContext,
  );
  await firstPutEntered.promise;
  const second = await worker.fetch(
    await uploadRequest(secondBytes, "comment_image"),
    env,
    executionContext,
  );
  releaseFirstPut.resolve();
  const firstResponse = await first;
  const replay = await worker.fetch(await uploadRequest(firstBytes, "comment_image"), env, executionContext);
  return { database, stored, deleted, firstResponse, second, replay, putCount };
}
...
async function runConcurrentManagedUploads(
  ...
  await acceptCommentUpload(database, env, { kind: "attachment", bytes: firstBytes });
  const first = worker.fetch(
    await uploadRequest(firstBytes, "attachment", firstSha),
    env,
    executionContext,
  );
  await firstPutEntered.promise;
  const second = await worker.fetch(
    await uploadRequest(secondBytes, "attachment", secondSha),
    env,
    executionContext,
  );
  releaseFirstPut.resolve();
  const firstResponse = await first;
  const replay = await worker.fetch(await uploadRequest(firstBytes, "attachment", firstSha), env, executionContext);
  return {
    database,
    stored,
    deletedUrls,
...
describe("concurrent Comment provider registration", () => {
  it("keeps the canonical R2 bytes when same-SHA uploads race for one item", async () => {
    ...
    expect(new Set(rows.map((row) => row.r2_key)).size).toBe(1);
    expect(rows.map((row) => row.status)).toEqual(["ready"]);
    expect(result.stored.size).toBe(1);
  ...
  it("rejects different-SHA same-size R2 bytes racing against the accepted item", async () => {
    ...
    expect(new Set(rows.map((row) => row.r2_key)).size).toBe(1);
    expect(new Set(rows.map((row) => row.sha256)).size).toBe(1);
    expect([...result.stored.values()]).toEqual([firstBytes]);
    ...
    expect(result.stored.get(winner.r2_key)).toEqual(firstBytes);
    result.database.close();
  });
  ...
  it("rejects different-SHA same-size managed bytes racing against the accepted item", async () => {
    ...
    expect(result.stored.get(winnerUrl)).toEqual(firstBytes);
    result.database.close();
  });
});
