...
describe("v11 durable R2 upload acceptance archive profile", () => {
  it("preserves the frozen V11 writer and restores pending, failed, ready, deduplicated and collected historical receipts without new byte roots", async () => {
    ...
    try {
      ...
      expect(restored.report).toMatchObject({ schemaVersion: 11, archiveProfile: "fp1-r2-upload-acceptance", appliedForwardMigrations: [{ name: "0005_metrology_reference_acceptance.sql" }, { name: "0006_comment_acceptance.sql" }, { name: "0007_fp1_file_authority_transition.sql" }, { name: "0008_fp1_shadow_runtime.sql" }], warnings: [],
