...
import type { FullExportManifestV15 as FullExportManifest } from "../shared/contracts/export";
...
describe("complete archive physical identity", () => {
  it("round-trips imported ASCII, Unicode, case and dot-segment locators through real export routes", async () => {
    ...
    try {
      ...
      const response = await request("/api/exports/all?archiveSchema=15&archiveWriter=1");
      expect(response.status).toBe(200);
      const manifest = await response.json() as FullExportManifest;
      const result = await buildFullExportArchiveV15(manifest, undefined,
      ...
      expect(archivedManifest.schemaVersion).toBe(15);
      ...
      const afterExport = await request("/api/exports/all?archiveSchema=15&archiveWriter=1");
  ...
  it("keeps managed objects distinct when record IDs or display names sanitize identically", async () => {
    ...
    try {
      ...
      expect(response.status).toBe(200);
      const manifest = await response.json() as FullExportManifest;
      const result = await buildFullExportArchiveV15(manifest, undefined, (async (url) => {
