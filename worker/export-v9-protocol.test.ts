...
describe("v9 dormant file registry archive profile", () => {
  it("retains the frozen historical browser contract and round-trips populated dormant observations and all legacy bytes", async () => {
    ...
    try {
      ...
      expect(restored.report).toMatchObject({ schemaVersion: 9, archiveProfile: "fp1-legacy-overlap", appliedForwardMigrations: [{ name: "0003_fp1_import_acceptance.sql" }, { name: "0004_r2_upload_acceptance.sql" }], warnings: [],
