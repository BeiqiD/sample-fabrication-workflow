...
describe("v10 durable import acceptance archive profile", () => {
  it("preserves the frozen V10 writer and restores populated legacy, pending, failed and ready receipts with exact bytes and restored write guards", async () => {
    ...
    try {
      ...
      expect(restored.report).toMatchObject({ schemaVersion: 10, archiveProfile: "fp1-import-acceptance", appliedForwardMigrations: [{ name: "0004_r2_upload_acceptance.sql" }, { name: "0005_metrology_reference_acceptance.sql" }, { name: "0006_comment_acceptance.sql" }, { name: "0007_fp1_file_authority_transition.sql" }, { name: "0008_fp1_shadow_runtime.sql" }], warnings: [],
