...
import {
  ...
  edgeEndpointRevisionSettlementProof,
  edgeIdentitySettlementProof,
  placementRevisionSettlementProof,
  projectItemIdentitySettlementProof,
  projectRevisionSettlementProof,
  type SettlementProof,
} from "./projects/settlement";
...
async function projectCall<T>(
  ...
  try {
    ...
    const failure = await classifyProjectFailure(error, settlementProof);
    if (failure) {
      throw new ProjectMutationHttpException(
        failure.code === "not_found" ? 404 : 409,
        failure.message,
        failure.authoritativeRejection,
      );
    }
    throw error;
  }
