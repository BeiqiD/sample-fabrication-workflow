...
function TemplateStepEditor({ template, step, onSaved }: { template: TemplateDetail; step: TemplateStepRecord; onSaved: () => Promise<void> }) {
  ...
  return <article className="card template-step-card">
    ...
    <div className="template-step-body">
      <div className="card-title-row"><div><h3 className="card-title">{step.name}</h3>{step.sectionName && <span className="section-label">{step.sectionName}</span>}</div>{!template.locked && !template.archived && <button type="button" className="text-button" onClick={() => setEditing((value) => !value)}>{editing ? "Cancel" : "Edit"}</button>}</div>
