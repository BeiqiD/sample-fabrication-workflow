...
describe("reversible Project Canvas paste rejection", () => {
  ...
  it("keeps the journal uncertain across a reversible 409 until the old request late-commits and exact replay proves settlement", async () => {
    let authoritative = pasteSnapshot();
    let getAttempts = 0;
    let referenceAttempts = 0;
    let edgeAttempts = 0;
    let failedReferenceBody: Record<string, any> | null = null;
    let lateCommitResult: ReturnType<typeof createProjectItemResponse>["result"] | null = null;
      ...
      if (path.endsWith("/items/reference")) {
        ...
        if (referenceAttempts === 1) {
          ...
          return Promise.reject(new TypeError(
            "simulated response loss while the old server request remains in flight",
          ));
        }
        ...
        if (!lateCommitResult) {
          throw new Error("The delayed original request has not committed yet");
        }
        return jsonResponse({ ...lateCommitResult, replayed: true }, 200);
      }
      if (path.endsWith("/edges")) {
        edgeAttempts += 1;
        throw new Error("The later edge step must remain unattempted during settlement");
      }
    ...
    lateCommitResult = lateCommit.result;
    ...
    expect(referenceAttempts).toBe(3);
