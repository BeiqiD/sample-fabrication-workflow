...
// directory or deployment configuration and does not qualify B for the S0 schema.
...
const migrations = readdirSync(new URL("migrations/", root))
  .filter((name) => name.endsWith(".sql")).sort().map((name) => read(`migrations/${name}`));
const schemas = {
...
VALUES ('bridge-retained-legacy', 'reference-step-a', 'individual',
...
let worker, predecessorWorker, contractedWorker;
// Reconstruct the exact reviewed E/B4 files without requiring Git history.
...
async function fixture(stage, engine) {
  ...
  const database = engine === "SQLite" ? hostD1(new DatabaseSync(":memory:")) : await runtime.getD1Database("DB", "bridge");
  for (const migration of migrations) await apply(database, migration);
  await apply(database, read("worker/fixtures/reference-graph.sql") + retainedSql);
  await apply(database, schemas.S1);
  const bucket = engine === "SQLite" ? hostBucket() : await runtime.getR2Bucket("ASSETS", "bridge");
  await bucket.put("reference/private/comment.png", new Uint8Array(10), { httpMetadata: { contentType: "image/png" } });
  const predecessorFetcher = await runtime.getWorker("predecessor");
  const contractedFetcher = await runtime.getWorker("contracted");
  const requestFor = (implementation, fetcher) => (path, method = "GET", body, headers) => {
    const init = { method, ...(body === undefined ? {} : headers
      ? { headers, body }
      : { headers: { "content-type": "application/json" }, body: JSON.stringify(body) }) };
    return engine === "SQLite"
      ? implementation.fetch(new Request(`https://app.test/api${path}`, init), { AUTH_MODE: "disabled", DB: database, ASSETS: bucket }, { waitUntil() {}, passThroughOnException() {} })
      : fetcher.fetch(`https://app.test/api${path}`, init);
  };
  const request = requestFor(worker, await runtime.getWorker("bridge"));
  const predecessorRequest = requestFor(predecessorWorker, predecessorFetcher);
  const contractedRequest = requestFor(contractedWorker, contractedFetcher);
  ...
  return { stage, database, request, predecessorRequest, contractedRequest, rows, one, targets, snapshot, close: () => database.native?.close() };
}
...
async function predecessorReads(f, ids, body) {
  const response = await json(await f.predecessorRequest("/references/resolve", "POST", {
    targets: ids.map((id) => ({ type: "comment_occurrence", id })),
  }), 200, "E predecessor resolves B occurrences");
  assert(response.results.every((result) => result.source.excerpt === (body || null)));
  const seen = new Set();
  for (const sample of ["reference-sample-a", "reference-sample-b"]) {
    const comments = await detail({ ...f, request: f.predecessorRequest }, sample);
    for (const comment of comments.filter(({ id }) => ids.includes(id))) {
      seen.add(comment.id);
      assert.equal(comment.body, body);
    }
  }
  assert.deepEqual([...seen].sort(), [...ids].sort(), "E Sample detail exposes every B occurrence");
}
...
async function qualifyRetainedReads(f) {
  ...
  assert.equal(comments.find(({ id }) => id === "bridge-retained-legacy").body, "Retained legacy observation 中文");
  assert.equal((await resolve(f, "reference-comment-occurrence-a")).source.excerpt, "Shared reference Comment body");
  assert.equal((await resolve(f, "bridge-retained-legacy")).source.excerpt, "Retained legacy observation 中文");
  assert.deepEqual(await search(f, "Shared reference Comment body"), ["reference-comment"]);
  assert.deepEqual(await search(f, "Retained legacy observation 中文"), ["bridge-retained-legacy"]);
...
async function qualifyLegacy(f, scope, imageOnly) {
  const body = imageOnly ? "" : `Blegacy${scope}observation 中文`;
  ...
  assert(occurrences.every((row) => row.body === body && row.body === row.legacy_body), "B dual-writes the same legacy text for the E reader overlap");
  const ids = occurrences.map(({ id }) => id);
  await predecessorReads(f, ids, body);
...
async function qualifyCanonical(f, scope, imageOnly) {
  const id = `bridge-canonical-${scope}-${imageOnly ? "image" : "text"}`;
  const itemId = `${id}-item`;
  const body = imageOnly ? "" : `Bcanonical${scope}observation 中文`;
  ...
  assert(occurrences.every((row) => row.body === ""), "B canonical finalization uses the schema placeholder only");
...
async function qualifyPredecessorWrites(f) {
  for (const scope of ["individual", "common"]) {
    const body = `E${scope}overlap`;
    const result = await json(await f.predecessorRequest("/run-step-comments", "POST", {
      scope, body, targets: await f.targets(scope),
    }), 201, "E predecessor creates legacy occurrences while B is present");
    const rows = await f.rows("SELECT * FROM run_step_comments WHERE operation_group_id = ? ORDER BY id", result.operationGroupId);
    assert.equal(rows.length, scope === "common" ? 2 : 1);
    assert(rows.every((row) => row.body === body && row.legacy_body === body));
    for (const row of rows) assert.equal((await resolve(f, row.id)).source.excerpt, body);
    const id = `bridge-e-${scope}-canonical`;
    await json(await f.predecessorRequest("/comment-submissions", "POST", {
      id, body: `${body}canonical`, context: { kind: "run_steps", scope, targets: await f.targets(scope) }, items: [],
    }), 201, "E predecessor creates canonical submission");
    await json(await f.predecessorRequest(`/comment-submissions/${id}/finalize`, "POST"), 200, "E predecessor finalizes canonical submission");
    const occurrences = await f.rows("SELECT * FROM run_step_comments WHERE submission_id = ? ORDER BY id", id);
    assert.equal(occurrences.length, scope === "common" ? 2 : 1);
    assert(occurrences.every((row) => row.body === `${body}canonical` && row.legacy_body === null));
    for (const row of occurrences) assert.equal((await resolve(f, row.id)).source.excerpt, `${body}canonical`);
  }
}

async function qualifyContractedOverlap(f) {
  for (const scope of ["individual", "common"]) {
    const create = async (request, text) => {
      const result = await json(await request("/run-step-comments", "POST", { scope, body: text, targets: await f.targets(scope) }), 201, "Create during B/C overlap");
      return f.rows("SELECT * FROM run_step_comments WHERE operation_group_id = ? ORDER BY id", result.operationGroupId);
    };
    const cRows = await create(f.contractedRequest, `Coverlap${scope}`);
    assert.equal(cRows.length, scope === "common" ? 2 : 1);
    assert(cRows.every((row) => row.body === "" && row.legacy_body === `Coverlap${scope}`));
    for (const row of cRows) assert.equal((await resolve(f, row.id)).source.excerpt, `Coverlap${scope}`);
    assert.equal((await detail(f)).find(({ id }) => cRows.some((row) => row.id === id)).body, `Coverlap${scope}`);
    const bRows = await create(f.request, `Boverlap${scope}`);
    assert(bRows.every((row) => row.body === `Boverlap${scope}` && row.legacy_body === row.body));
    for (const row of bRows) assert.equal((await resolve({ ...f, request: f.contractedRequest }, row.id)).source.excerpt, `Boverlap${scope}`);
    assert.equal((await detail({ ...f, request: f.contractedRequest })).find(({ id }) => bRows.some((row) => row.id === id)).body, `Boverlap${scope}`);
    await create(f.contractedRequest, `CafterB${scope}`);
    assert.deepEqual(await f.rows("SELECT * FROM run_step_comments WHERE operation_group_id = ? ORDER BY id", cRows[0].operation_group_id), cRows, "B writes preserve C text and occurrence identities");
    assert.deepEqual(await f.rows("SELECT * FROM run_step_comments WHERE operation_group_id = ? ORDER BY id", bRows[0].operation_group_id), bRows, "C writes preserve B text and occurrence identities");
  }
}
...
async function qualifyGuards(f) {
  ...
  await json(await f.request("/comment-submissions", "POST", { id: "bridge-stale-targets", body: "Must not partially write", context: { kind: "run_steps", scope: "common", targets: stale }, items: [] }), 409, "Reject stale canonical write");
  assert.deepEqual(await f.snapshot(), before, "stale target guards preserve all rows");
  await json(await f.request("/comment-submissions", "POST", { id: "bridge-trash-targets", body: "Must not finalize after trash", context: { kind: "run_steps", scope: "common", targets: await f.targets("common") }, items: [] }), 201, "Create pending before target trash");
  ...
  await json(await f.request("/comment-submissions/bridge-trash-targets/finalize", "POST"), 409, "Reject finalization after target trash");
...
before(async () => {
  const bundleOptions = { write: false, bundle: true, format: "esm", platform: "browser", target: "es2022",
    conditions: ["workerd", "worker", "browser"], logLevel: "silent", nodePaths: [fileURLToPath(new URL("node_modules", root))] };
  const bundle = await build({ ...bundleOptions, entryPoints: [fileURLToPath(new URL("worker/index.ts", root))] });
  const script = bundle.outputFiles[0].text;
  worker = (await import(`data:text/javascript;base64,${Buffer.from(script).toString("base64")}`)).default;
  async function reconstruct(label, fileCount) {
    const sources = JSON.parse(read(`scripts/fixtures/backend-bridge/${label}-source-hashes.json`));
    const source = mkdtempSync(join(tmpdir(), `bridge-${label}-`));
    try {
      assert.equal(sources.version, 1);
      const sourcePaths = Object.keys(sources.files);
      assert.equal(sourcePaths.length, fileCount);
      ...
      execFileSync("git", ["apply", "--whitespace=error-all", fileURLToPath(new URL(`scripts/fixtures/backend-bridge/restore-${label}.patch`, root))], { cwd: source });
      const replacements = new Map(sourcePaths.map((path) => {
        const contents = readFileSync(join(source, path), "utf8");
        assert.equal(createHash("sha256").update(contents).digest("hex"), sources.files[path], `exact ${label} source: ${path}`);
        return [fileURLToPath(new URL(path, root)), contents];
      }));
      return (await build({ ...bundleOptions, entryPoints: [fileURLToPath(new URL("worker/index.ts", root))], plugins: [{
        name: `verified-${label}-sources`,
        setup(bundler) { bundler.onLoad({ filter: /\.ts$/ }, ({ path }) => replacements.has(path)
          ? { contents: replacements.get(path), loader: "ts", resolveDir: dirname(path) } : undefined); },
      }] })).outputFiles[0].text;
    } finally { rmSync(source, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 }); }
  }
  const predecessorScript = await reconstruct("e-reader", 5);
  const contractedScript = await reconstruct("c-writer", 5);
  predecessorWorker = (await import(`data:text/javascript;base64,${Buffer.from(predecessorScript).toString("base64")}`)).default;
  contractedWorker = (await import(`data:text/javascript;base64,${Buffer.from(contractedScript).toString("base64")}`)).default;
  const definition = (name, sourceScript) => ({ name, modules: true, script: sourceScript, compatibilityDate: "2026-07-20",
    bindings: { AUTH_MODE: "disabled" }, d1Databases: { DB: "bridge-qualification" }, r2Buckets: { ASSETS: "bridge-images" } });
  runtimes.set("S1", new Miniflare({ workers: [definition("bridge", script), definition("predecessor", predecessorScript), definition("contracted", contractedScript)], log: new Log(LogLevel.ERROR) }));
});
after(async () => { await Promise.all([...runtimes.values()].map((runtime) => runtime.dispose())); });

for (const stage of ["S1"]) for (const engine of ["SQLite", "workerd D1"]) {
  test(`B actual Worker APIs preserve the E overlap on ${stage} / ${engine}`, { timeout: 60_000 }, async () => {
    ...
    try {
      ...
      await qualifyPredecessorWrites(f);
      await qualifyContractedOverlap(f);
