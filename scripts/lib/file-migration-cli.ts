...
import { validateFullExportV10 } from "../../shared/contracts/export-protocol";
...
export async function planFileMigrationSnapshot(input: { snapshotPath: string; outputPath: string }) {
  ...
  let manifest;
  try { manifest = await validateFullExportV10(parsed); }
  catch { throw new Error("Snapshot does not satisfy the complete schema-10 export contract"); }
