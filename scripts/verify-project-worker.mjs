...
try {
  ...
  const exportResponse = await miniflare.dispatchFetch("https://app.test/api/exports/all?archiveSchema=13&archiveWriter=1");
  const fullExport = await exportResponse.json();
  assert.equal(exportResponse.status, 200, JSON.stringify(fullExport));
  assert.equal(fullExport.schemaVersion, 13);
  assert.equal(fullExport.archiveProfile, "fp1-comment-acceptance");
