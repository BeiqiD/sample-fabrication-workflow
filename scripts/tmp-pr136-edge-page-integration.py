...
def one(value: str, occurrence: int = 0):
    found = matches(value)
    if len(found) <= occurrence:
        raise SystemExit(f"missing line {value!r}; found {len(found)}")
    return found[occurrence]


def insert_before(value: str, additions: list[str], occurrence: int = 0):
    index = one(value, occurrence)
    lines[index:index] = additions


def insert_after(value: str, additions: list[str], occurrence: int = 0):
    index = one(value, occurrence)
    lines[index + 1:index + 1] = additions


def replace_line(value: str, replacements: list[str], occurrence: int = 0):
    index = one(value, occurrence)
    lines[index:index + 1] = replacements


insert_before('import { projectReferenceRemovalNeedsReconciliation } from "../lib/project-reference-removal";', [
...
replace_line(
    '      || attachmentEditor !== null)',
    ['      || attachmentEditor !== null', '      || edgeController.unsafe)'],
    0,
)
replace_line(
...
replace_line(
    ...
    ['      && attachmentEditorRef.current === null', '      && !edgeController.unsafeRef.current) return;'],
)
insert_after('    clearOwnedContentState();', ['    edgeController.resetForAuthoritativeReload();'])
replace_line(
...
replace_line(
    ...
    ['      || attachmentEditor !== null', '      || edgeController.unsafe) return;'],
)
replace_line(
...
common_guard = '      || markdownEditorRef.current || pendingAttachmentRef.current || attachmentEditorRef.current) return;'
common_replacement = [
    '      || markdownEditorRef.current || pendingAttachmentRef.current || attachmentEditorRef.current',
    '      || edgeController.unsafeRef.current) return;',
]
while common_guard in lines:
    replace_line(common_guard, common_replacement)

index = one('      || projectGeometryEquals(command.before, command.after)) return;')
lines[index:index] = ['      || edgeController.unsafeRef.current']
...
undo_start = next(i for i, line in enumerate(lines) if line.startswith('  const undo = useCallback'))
undo_end = next(
    i for i, line in enumerate(lines[undo_start:], undo_start)
    if line.startswith('  const mergeReferenceInsertion = useCallback')
)
history_lines = [
...
lines[undo_start:undo_end] = history_lines
...
select_start = one('  const selectProjectItem = useCallback((itemId: string | null) => {')
select_end = one('  const descriptors = useMemo(() => snapshot ? projectMapNodes(snapshot).map((node) => ({')
select_lines = [
...
lines[select_start:select_end] = select_lines

insert_before('  const reloadAfterReferenceConflict = useCallback(() => {', [
...
replace_line('  const ownedContentBusy = Boolean(markdownEditor || pendingAttachment || attachmentEditor);', [
...
replace_line('    || ownedContentBusy;', ['    || workspaceOperationBusy;'])

nav_index = one('  const navigationBlockMessage = pendingReferenceRemoval')
lines[nav_index:nav_index + 1] = [
...
page_text = page_text.replace(
    '!pendingReference && !pendingReferenceRemoval && !ownedContentBusy',
    '!pendingReference && !pendingReferenceRemoval && !workspaceOperationBusy',
)
page_text = page_text.replace(
...
page_text = page_text.replace(
    '|| Boolean(pendingReferenceRemoval) || ownedContentBusy}',
    '|| Boolean(pendingReferenceRemoval) || workspaceOperationBusy}',
)
lines = page_text.splitlines()

banner_index = one('    {blocker.state === "blocked" && <div className="project-save-banner warning" role="alertdialog" aria-label="Unsaved Project changes">')
edge_banner = [
...
lines[banner_index:banner_index] = edge_banner
insert_after('        <button type="button" className="button compact-button" onClick={stayOnProject}>Stay on Project</button>', [
    '        {edgeController.pending?.status === "uncertain" && <button type="button" className="button primary compact-button" onClick={edgeController.retryExact}>Retry exact edge operation</button>}',
...
insert_after('            nodes={descriptors}', [
    '            edges={snapshot.edges}',
    '            pendingEdge={edgeController.pendingEdge}',
])
insert_after('            selectedItemId={selectedItemId}', ['            selectedEdgeId={edgeController.selectedEdgeId}'])
insert_after('            onSelect={selectProjectItem}', [
    '            onEdgeSelect={selectProjectEdge}',
    '            onEdgeConnect={edgeController.connect}',
])

insert_after('  const selectedItem = snapshot?.items.find((item) => item.id === selectedItemId) ?? null;', [
...
inspector_index = one('        {selected ? <div className="project-inspector-content">')
edge_inspector = [
...
lines[inspector_index:inspector_index + 1] = edge_inspector
replace_line(
...
hook_path = Path("src/lib/use-project-edge-controller.ts")
hook_lines = hook_path.read_text().splitlines()
start = next(i for i, line in enumerate(hook_lines) if line.startswith('  const cancelEdit = useCallback'))
end = next(i for i, line in enumerate(hook_lines[start:], start) if line.startswith('  const saveEdit = useCallback'))
hook_lines[start:end] = [
