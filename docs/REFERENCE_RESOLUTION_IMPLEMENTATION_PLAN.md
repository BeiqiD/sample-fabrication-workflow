...
Status: implementation contract for the next v3 backend slice

Last reviewed: 2026-08-08 after PR #124 on `v2/backend-foundation`

This document defines the exact scope for the first reference-registry and
...
2. a sparse, idempotent `reference_targets` registry;
3. one bounded batch resolver for every current referenceable source type;
4. a read-only API endpoint that preserves caller order and partial failures;
5. an internal registration service for later Project-item creation;
6. one mapping from public target types to permanent-delete blocker types;
7. export, migration, CI, and D1/workerd coverage for the new foundation.
...
Each requested target resolves to one uniform result:
...
Each target type has one adapter that accepts a batch of IDs and issues bounded
source-specific SQL. Adapters must:
...
1. validates at most 200 targets;
2. groups and deduplicates IDs by type;
3. runs at most one source query for each type present;
4. performs one registry lookup for the requested targets;
5. restores original request order and duplicate entries;
6. reports missing targets per item without failing unrelated results.

The expected query count is bounded by the number of target types, not by the
number of targets.
...
1. resolves the target through the same source adapter;
...
- 1 to 200 targets;
- unknown type, empty ID, or excessive ID length returns `400`;
...
The export must include every `reference_targets` row exactly, including IDs,
validation timestamps, tombstone metadata, and context JSON. Registry rows do
not create blob occurrences or download entries.
...
`verify:v3-deployment` must include the reference-foundation gate before any
remote migration or deployment command can run.
...
- `PROJECT_DESIGN_FOUNDATION.md`: Phase 1B complete after PR #123/#124 and Phase
  2 split into registry/resolution, deep links, and deterministic search;
...
- `ARCHITECTURE.md`: read-only reference boundary and query-count invariant;
...
3. all nine target classes resolve in bounded batches;
4. soft-deleted sources and deleted ancestors remain resolvable;
5. common Comments retain every deterministic context;
6. no resolver response exposes a physical storage locator;
7. the public endpoint is read-only and order-preserving;
8. complete export preserves registry rows;
9. host SQLite, Wrangler local D1/workerd, focused tests, the complete test suite,
   and production build pass;
10. the PR targets only `v2/backend-foundation`;
...
3. Project-owned data, Project items, Text, and Inspector;
