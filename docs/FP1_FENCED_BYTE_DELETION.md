...
| Import recovery | `fabublox-recovery-assets.ts`, `fabublox-import-recovery.ts`, import routes and publication helpers; retain owning-import visibility, canonical winners and failed/private ownership transfer. [FP1e](./FP1_RECOVERY_BYTE_VERIFICATION.md) adds complete-byte verification to independent recovery; the File authority conversion remains required. |
