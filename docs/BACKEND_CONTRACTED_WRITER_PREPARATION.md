...
The original C preparation left the active migration directory unchanged. This
successor candidate selects the reviewed S2 baseline for new empty local
databases; see [the final S2 application gate](BACKEND_S2_APPLICATION_GATE.md).
It still must not be deployed to the existing S0 service.
...
the unchanged archived historical chain followed by the inactive stage SQL only in disposable
...
This successor candidate includes the explicit qualification in its full
`verify:ci` gate and selects S2 for current fixtures and application smokes. The
separate `backend-compatibility-schema.test.mjs` preserves the historical pre-C
E/B4 writer assertions on S1 by reconstructing the exact reviewed source files.
The final integration still requires deployment retirement evidence and browser
