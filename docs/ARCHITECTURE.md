...
- [Accepted ordinary/Project uploads](./FP1_DURABLE_R2_UPLOAD_ACCEPTANCE.md) freeze one byte-upload operation. [Metrology reference acceptance](./FP1_METROLOGY_REFERENCE_ACCEPTANCE.md) also atomically publishes the business occurrence and its immutable receipt. Replays validate current lifecycle and bytes without restoring later deletions; neither ledger adds retention roots or activates File authority.
- [Comment acceptance](./FP1_COMMENT_ACCEPTANCE.md) preserves canonical submissions/items/targets and adds immutable request and upload history. Hashes, physical profiles and candidates are frozen before provider work; one execution owner and fixed seven-day deadline govern retries. Cancellation fences late completion. Finalization checks every retained item and target and atomically records all occurrences/events and its original result.
- [The additive File-authority transition](./FP1_FILE_AUTHORITY_TRANSITION.md) installs typed consumer/conversion substrate and schema-14 recovery under immutable `legacy` mode. Existing locator reads, writes, global-SHA reuse, retention, quarantine and GC remain authoritative. The next PR supplies shadow writers, a resolver and a conversion ledger; a later atomic activation must fence old Workers and switch all lifecycle authority together. Cross-purpose consumers cannot share one active placement without independently verified copies.
...
- The complete browser archive uses [schema 14 / writer 1, profile `fp1-file-authority-transition`](./FP1_FILE_AUTHORITY_TRANSITION.md), including import, byte-upload, metrology and Comment publication receipts, the original registry tables, and the additive authority-transition state. The same batch observes physical schema and retired compatibility values; the archive validates and preserves their provenance. Historical schema-7 through schema-13 archives retain their frozen validators and remain supported by isolated offline recovery before reviewed forward upgrades. Historical receipts and transition rows are validated without restarting provider work, activating File authority, or requiring their original business references or provider bytes to remain available.
