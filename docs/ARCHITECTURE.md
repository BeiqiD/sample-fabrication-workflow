...
- The complete browser archive uses [schema 12 / writer 1, profile `fp1-metrology-reference-acceptance`](./FP1_METROLOGY_REFERENCE_ACCEPTANCE.md), including import, byte-upload and metrology publication receipts plus the four dormant file-registry tables. The same batch observes physical schema and retired compatibility values; the archive validates and preserves their provenance. Historical schema-7 through schema-11 archives retain their frozen validators and remain supported by isolated offline recovery before reviewed forward upgrades. Historical receipts are validated without requiring their original business references or provider bytes to remain available.
