...
- The complete browser archive uses [schema 13 / writer 1, profile `fp1-comment-acceptance`](./FP1_COMMENT_ACCEPTANCE.md), including import, byte-upload, metrology and Comment publication receipts plus the four dormant file-registry tables. The same batch observes physical schema and retired compatibility values; the archive validates and preserves their provenance. Historical schema-7 through schema-12 archives retain their frozen validators and remain supported by isolated offline recovery before reviewed forward upgrades. Historical receipts are validated without requiring their original business references or provider bytes to remain available.
